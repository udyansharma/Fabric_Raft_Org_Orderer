...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.org1.example.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.org1.example.com
    docker cp -a orderer.org1.example.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.org1.example.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.org1.example.com
